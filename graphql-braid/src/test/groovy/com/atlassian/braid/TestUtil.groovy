...
class TestUtil {
    ...
    static TypeDefinitionRegistry typeRegistry(String schema) {
        return new SchemaParser().parse(schema)
    }

    static GraphQLSchema schema(TypeDefinitionRegistry registry) {
        def options = SchemaGenerator.Options.defaultOptions().enforceSchemaDirectives(false)
        return new SchemaGenerator().makeExecutableSchema(options, registry, mockRuntimeWiring)
    }
    ...
    static RuntimeWiring.Builder mockRuntimeWiringBuilder = RuntimeWiring.newRuntimeWiring().wiringFactory(mockWiringFactory)
    static RuntimeWiring mockRuntimeWiring = mockRuntimeWiringBuilder.build()
